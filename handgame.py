...
while True:
    ...
    while True:
        ...
        if not game_started:
            ...
            cv2.circle(frame, (player_x, player_y), 20,
                       (255, 0, 0), -1)
        ...
        if distance < PLAYER_RADIUS + coin_radius:
            ...
            # Diminui tamanho da moeda
            coin_radius = max(8, 20 - score)

            # Nova posição da moeda
            coin_x = random.randint(50, frame.shape[1] - 50)
            coin_y = random.randint(50, frame.shape[0] - 50)

        # Desenho dos elementos
        cv2.circle(frame, (coin_x, coin_y), coin_radius,
                   (0, 255, 255), -1)

        cv2.circle(frame, (player_x, player_y), PLAYER_RADIUS,
                   (255, 0, 0), -1)
